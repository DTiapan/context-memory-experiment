...
def _row(size, question, result, by_id):
    return _metrics_row(
        size, question, result.strategy, result.chunk_ids, result.candidate_count,
        [by_id[i] for i in result.chunk_ids], result.retrieval_rounds, result.latency_ms
    )


def _or_row(size, question, memories, by_id):
    start = perf_counter()
    ...
    return _metrics_row(
        size, question, "or_ranked", retrieved_ids, result.candidate_count,
...
def _hierarchical_row(size, question, index, by_id):
    ...
    return _metrics_row(
        size, question, "hierarchical", retrieved_ids, candidate_set.count,
        scored, 2, latency_ms
    )


def _question_rows(size, question, memories, index, hierarchical_index, by_id):
    return [
        _row(size, question, fixed_top_k(question, memories), by_id),
        _row(size, question, iterative(question, memories), by_id),
        _row(size, question, indexed_iterative(question, index), by_id),
        _or_row(size, question, memories, by_id),
...
def run_scale_point(size, questions, memories, progress=None, max_question_workers=1):
    """Run one corpus-size point.

    Retrieval algorithms and scoring are unchanged. Question-level concurrency only
    changes execution scheduling; results are sorted back into the original question
    order so the benchmark output remains deterministic.
    """
    ...
    with ThreadPoolExecutor(max_workers=worker_count) as executor:
        futures = {
            executor.submit(_question_rows, size, question, memories, index, hierarchical_index, by_id):
                (number, question.id)
...
def _run_scale_point_worker(size, questions, corpus_builder, max_question_workers=1):
    ...
    rows = run_scale_point(
        size, questions, memories, progress=report,
...
def summarize(rows):
    ...
    for (size, strategy), group in sorted(groups.items()):
        output.append({
            "corpus_size": size, "strategy": strategy, "questions": len(group),
...
def run_scaling_experiment(sizes, questions, corpus_builder, max_workers=3, show_progress=True):
    ...
    if show_progress:
        print(
            f"[scale] requested_workers={requested_workers} | corpus_jobs={len(sizes)} | "
            f"corpus_workers={corpus_workers} | question_workers_per_corpus={question_workers}",
            file=sys.stderr, flush=True,
        )
        if requested_workers > len(sizes):
            print(
                f"[scale] question-level parallelism enabled to use the remaining worker budget "
                f"without changing retrieval algorithms",
                file=sys.stderr, flush=True,
            )
    ...
    with ProcessPoolExecutor(max_workers=corpus_workers) as executor:
        futures = {
            executor.submit(
                _run_scale_point_worker, size, questions, corpus_builder, question_workers
            ): size
            for size in sizes
        }
        completed = 0
        for future in as_completed(futures):
            ...
            if show_progress:
                print(
                    f"[scale] completed corpus={size:,} ({completed}/{len(sizes)})",
                    file=sys.stderr, flush=True,
                )
    ...
    return (
        [row for size in sizes for row in rows_by_size[size]],
